...
public class QuickSelect {
    /*
    * Find Kth Largest Element in Array -> O(n)
    *
    *
    * Note:
    ...
    如果 quick select / quick Basic_Algorithm.sort 写成了nums[i] < pivot 在左侧，nums[i] >= pivot 在右侧这种形式，就会导致划分不平均，从而导致错误或者超时。
    ...
    总结
    简单的说就是，quick select 和 quick Basic_Algorithm.sort 的 partition 目标不是将数组 严格的按照 nums[i] < pivot 和nums[i] >= pivot 去拆分开，而是只要能够让左半部分 <= 右半部分即可。这样子 nums[i] == pivot 放在哪儿都无所谓，两边都可以放。
    * */
