...
public class Sorting {
        ...
        while(sorted_flag) {
            ...
            for(int j=0; j<length-i-1;j++) {
                ...
                if(target[j] > target[j+1]) {
                    ...
                    sorted_flag = true; // if Basic_Algorithm.sort operation happen in this loop, then change sorted_flag = true
                }
            }
            i++;
        }
    ...
    public void insertSort(int[] target) {
        ...
        // each loop add new element into Basic_Algorithm.sort array
        for(int i=0; i<length; i++) {
            for(int j=i; j>0; j--) {
                // check new element with Basic_Algorithm.sort array, and insert from end to head one by one.
